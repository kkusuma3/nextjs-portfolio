...
export default function Nav() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            window.pageYOffset > 0 ? setIsMenuClicked(false) : null;
            window.pageYOffset > window.innerHeight ? setIsScrolled(true) : setIsScrolled(false);
        }
    ...
    return (
        <Headroom>
            <nav
                id="nav"
                className={navClassName(isScrolled, isMenuClicked, "transition duration-500 bg-transparent mb-10 lg:px-24")}
            >
                <ul className="flex flex-wrap w-full justify-between items-center px-4 md:px-8 py-4 md:mb-4">
                    <li>
                        <Link href="/">
                            <a className="text-maroon text-4xl font-medium">{STRING_HOME_TITLE}</a>
                        </Link>
                    </li>
